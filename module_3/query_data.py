...
avg_gpa_us_fall2025 = one("""
    ...
    WHERE (us_or_international ILIKE 'us' OR us_or_international ILIKE 'american')
...
fall_total = one("SELECT COUNT(*) FROM applicants WHERE term ILIKE '%%Fall 2025%%'")
fall_accept = one("""
    SELECT COUNT(*) FROM applicants
...
avg_gpa_fall_accept = one("""
    SELECT AVG(gpa) FROM applicants
    WHERE term ILIKE '%%Fall 2025%%' AND status ILIKE 'accept%%' AND gpa IS NOT NULL
""")
print(f"6) Avg GPA of Fall 2025 Acceptances: {avg_gpa_fall_accept}")
...
jhu_ms_cs = one("""
    SELECT COUNT(*) FROM applicants
    WHERE (llm_generated_university ILIKE '%%johns hopkins%%' OR llm_generated_university ILIKE '%%jhu%%')
